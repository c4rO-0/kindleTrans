...
function showBar(data) {
    ...
    if (data.unprocNailInfo.n_sell + data.cutNailInfo.n_sell > 0) {
        ...
        p_sell_nail_low = Math.min(
            data.unprocNailInfo.p_sell_low == null ? -1. : data.unprocNailInfo.p_sell_low, 
            data.cutNailInfo.p_sell_low  == null ? -1. : data.cutNailInfo.p_sell_low)

        p_sell_nail_high = Math.max(
            data.unprocNailInfo.p_sell_high == null ? 1.E8 : data.unprocNailInfo.p_sell_high, 
            data.cutNailInfo.p_sell_high  == null ? 1.E8 : data.cutNailInfo.p_sell_high)
    }
    ...
    n_eth = data.marketInfo.balance.n_eth_f + data.marketInfo.balance.n_eth_t
    $('#n-eff-prg .progress-bar:eq(0)').css('width', n_eth/n_max*100. +'%')
    $('#n-eff-prg .progress-bar:eq(0)').text(n_eth.toFixed(6))

    n_btc2eth = n_eff - n_eth
    $('#n-eff-prg .progress-bar:eq(1)').css('width', (n_btc2eth/n_max*100.) +'%')
    $('#n-eff-prg .progress-bar:eq(1)').text(n_btc2eth.toFixed(6))

    $('#n-eff-prg .progress-bar:eq(2)').css('width', (100.-n_eff/n_max*100.) +'%')
    $('#n-eff-prg .progress-bar:eq(2)').text((n_max-n_eff).toFixed(6))
